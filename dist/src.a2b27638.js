...
var svg = (0, _d.select)('#root').append('svg').attr('width', width + margin.left + margin.right).attr('height', height + margin.top + margin.bottom).append('g').attr('transform', "translate(".concat(margin.left, ",").concat(margin.top, ")"));

// X axis
var x = (0, _d.scaleBand)().range([0, width]).domain(_data.default.map(function (d) {
  return d.country;
})).padding(0.2);
svg.append('g').attr('transform', "translate(0, ".concat(height, ")")).call((0, _d.axisBottom)(x)).selectAll('text').attr('transform', 'translate(-10,0)rotate(-45)').style('text-anchor', 'end');

// Add Y axis
var y = (0, _d.scaleLinear)().domain([0, 13000]).range([height, 0]);
svg.append('g').call((0, _d.axisLeft)(y));

// Bars
svg.selectAll('mybar').data(_data.default).join('rect').attr('x', function (d) {
  return x(d.country);
}).attr('y', function (d) {
  return y(d.value);
...
},{"d3":"node_modules/d3/index.js","./data.json":"src/data.json"}],"node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
